...
/**
 * Класс описывающий окно клиента
 * @author Nik
 */
...
add(userSpis);
    ...
    /**
     * Доступные команды:
     * /addconf [conf] - создать конференцию
     * /joinconf [conf] - вступить в конференцию
     * @param evt
     */
    ...
    /**
     * Метод обновления списков пользователей и конференций
     */
    ...
    /**
     * При выборе конференции метод проходит по конференциям и открывает необходимую
     */
    public void changed() {
    boolean gege=false;
    aga:  
    for(int iai=0;iai<my_confs.size();iai++){
        ...
        if(gege)//найдена конференция
            my_confs.get(iai).my_textarea.append("eto chat"+my_confs.get(iai).konfName+"\n"); //в чат выводится ее имя
        if(!gege){
            my_confs.get(iai).my_textarea.setBounds(27, 23, 330, 170);
            add(my_confs.get(iai).my_textarea);
        }}
        ...
        out.setVisible(false);        
            for (int iai=0;iai<my_confs.size();iai++)
            {
            if((my_confs.get(iai).konfName).equals(konfa)){//при переходе в конференцию выводятся пользователи находящиеся в ней
                userSpis.removeAll();
    ...
    /**
     * Список пользователей в текущей конференции
     */
